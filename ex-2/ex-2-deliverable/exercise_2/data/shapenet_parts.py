...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        points_path = Path(ShapeNetParts.dataset_path / category_id / "points" /f"{shape_id}.pts")
        with open(points_path, 'rb') as p_f:
            points = np.loadtxt(p_f, dtype=np.float32)
            
        label_path = Path(ShapeNetParts.dataset_path / category_id / "points_label" /f"{shape_id}.seg")
        with open(label_path, 'rb') as l_f:
            labels = np.loadtxt(l_f, dtype=np.int64)
        sampled = np.random.choice(len(points), size=1024)
        sampled_points = points[sampled]
        sampled_labels = labels[sampled]
        return sampled_points.T, sampled_labels
