...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            # batch_indices = torch.multinomial(self.num_samples, replacement=False)
            batch_indices = random.choices(range(len(points)), k=self.num_samples)
            ...
            predicted_sdf = torch.clip(predicted_sdf, min=-self.truncation_distance, max=self.truncation_distance)
