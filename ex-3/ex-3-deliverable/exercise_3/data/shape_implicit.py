...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        prob = self.num_sample_points//2
        pos_indices = (torch.rand(prob)*pos_tensor.shape[0]).long()
        neg_indices = (torch.rand(prob)*neg_tensor.shape[0]).long()
        
        pos_samples = torch.index_select(pos_tensor, 0, pos_indices)
        neg_samples = torch.index_select(neg_tensor, 0, neg_indices)
        return torch.cat([pos_samples, neg_samples], 0)
